function velocities = InducedVelocitiesFMM2dgpu(blobs,targets,ksigmasqr)
% InducedVelocitiesFMM2dgpu computes induced velocities at target points using
% a fast multipole method on a gpu.
%
...
%   $ Revision: 1.0 $  $ Date: 2013/06/11 $
    ...
    velocitiesFMMgpuTemp = fmm2dgpu(blobsImag,blobs(3,:),targetsImag,'ndirect',35,'smooth','oseen','xopt',xopt,'cutoff',cutoff,'pot',1)/(2*pi);
    
    % convert velocities into the correct format
    velocities = zeros([2,length(velocitiesFMMgpuTemp)]);
    velocities(1,:) = -imag(velocitiesFMMgpuTemp);
    velocities(2,:) = -real(velocitiesFMMgpuTemp);
end